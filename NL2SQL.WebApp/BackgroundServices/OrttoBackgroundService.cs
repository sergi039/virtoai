...
{
    public class OrttoBackgroundService : BackgroundService
    {
        ...
        public OrttoBackgroundService(IServiceScopeFactory scopeFactory, IMapper mapper)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                var settings = await unitOfWork.Settings.GetOrttoSettingAsync();
        ...
        private async Task ExecuteTaskAsync(CancellationToken cancellationToken)
        {
            using var scope = _scopeFactory.CreateScope();
            var unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();
            var orttoImportService = scope.ServiceProvider.GetRequiredService<IOrttoImportService>();
            var setting = await unitOfWork.Settings.GetOrttoSettingAsync();
            ...
            var options = _mapper.Map<OrttoImportOptionsModel>(setting);
            var result = await orttoImportService.ImportOrttoDataAsync(options);

            await unitOfWork.Settings.UpdateLastSyncDataServiceAsync("Ortto", setting.Id, DateTime.UtcNow, result.TotalRecords);
        }
