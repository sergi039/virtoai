...
{
    public class ApolloBackgroundService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                var settings = await unitOfWork.Settings.GetApolloSettingAsync();
        ...
        private async Task ExecuteTaskAsync(CancellationToken cancellationToken)
        {
            using var scope = _scopeFactory.CreateScope();
            var unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();
            var apolloImportService = scope.ServiceProvider.GetRequiredService<IApolloImportService>();
            var setting = await unitOfWork.Settings.GetApolloSettingAsync();
            ...
            var options = _mapper.Map<ApolloImportOptionsModel>(setting);
            var result = await apolloImportService.ImportApolloDataAsync(options);

            await unitOfWork.Settings.UpdateLastSyncDataServiceAsync("Apollo", setting.Id, DateTime.UtcNow, result.TotalRecords);
        }
